...
`getSimPose` is a `Pose2dSupplier` passed to the `VisionSystem` constructor. `Robot.java` declares only a `visionSystem` field of type `VisionSystem` — the class is defined in `VisionSystem.java` but never constructed, so the sim camera isn't wired up yet. When you construct it, pass an equivalent pose supplier or the sim goes silent.
