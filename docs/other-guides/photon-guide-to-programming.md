...
Before adding a new mechanism, look at how an existing one is built. [`Hood.java`](../../src/main/java/frc/robot/subsystems/hood/Hood.java) is 232 lines and a good template. You'll see the single-file layout (subsystem, config as inner class, `WantedState`/`SystemState` machine), how gain slots are set up, and how `DoubleSupplier` is used for setpoints.
