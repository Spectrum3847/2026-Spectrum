...
We don't currently run a fuel-detection pipeline. PhotonVision on an Orange Pi was a 2025 experiment for that; the option to revive it for 2026 is open. `VisionSystem.java` (source: `src/main/java/frc/robot/subsystems/vision/VisionSystem.java`, verified 2026-08-03) declares a placeholder `PhotonCamera` ("cameraName") and uses `VisionSystemSim` for sim pose injection — no real-camera PhotonVision wrapper exists in `spectrumLib`.
