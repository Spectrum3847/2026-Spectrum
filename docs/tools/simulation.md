...
You can also visualize flying fuel projectiles: `RobotSim.mapleSimCreateFuelProjectile()` (source: `src/main/java/frc/robot/sim/`; verified 2026-08-03) attaches a trajectory-display callback that logs the projectile path to `SimShot/FuelProjectileSuccessfulShot` / `SimShot/FuelProjectileUnsuccessfulShot` (DogLog), and you can drag those entries into the 3D field to watch shots in flight.
