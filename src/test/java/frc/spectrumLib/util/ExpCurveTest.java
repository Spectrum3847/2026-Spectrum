...
public class ExpCurveTest {
    ...
    @Test
    @DisplayName("Test ExpCurve calculate with combined deadzone, exponent, scalar, and offset")
    void testCalculateCombinedPipeline() {
        double expVal = 3.0;
        double offset = 0.5;
        double scalar = 2.0;
        double deadzone = 0.2; // deadRadius = 0.1
        ExpCurve expCurve = new ExpCurve(expVal, offset, scalar, deadzone);

        // Within the deadband, the deadzone stage zeroes the input before the exponent is applied,
        // so only the offset stage contributes to the result.
        assertEquals(offset, expCurve.calculate(0.0), 1e-6);
        assertEquals(offset, expCurve.calculate(0.05), 1e-6);

        // Outside the deadband: deadzone -> (1/(1-0.1)) * (0.55-0.1) = 0.5, then exponent, scalar,
        // offset are applied in sequence.
        double deadzoned = (1.0 / (1.0 - 0.1)) * (0.55 - 0.1);
        double exponentiated = (Math.pow(expVal, Math.abs(deadzoned)) - 1.0) / (expVal - 1.0);
        double expected = exponentiated * scalar + offset;
        assertEquals(expected, expCurve.calculate(0.55), 1e-6);

        double expectedNegative = -exponentiated * scalar + offset;
        assertEquals(expectedNegative, expCurve.calculate(-0.55), 1e-6);
    }
}
