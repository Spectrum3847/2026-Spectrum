...
public class BumpPhysicsSim {
    ...
    private final double BUMP_KICK_SCALAR = 0.7;

    private final double halfLength = RobotSim.getSimRobotLength() / 2.0;
    private final double halfWidth = RobotSim.getSimRobotWidth() / 2.0;
    ...
    private TerrainState getTerrainState(Translation2d pos) {
        Translation2d fieldAgnosticPos = FieldHelpers.flipIfRedSide(pos);
        if (!(Field.LeftBump.area.contains(fieldAgnosticPos)
                || Field.RightBump.area.contains(fieldAgnosticPos))) {
        ...
        if (pos.getX() < Field.fieldLength / 2.0) {
            distFromInit = pos.getX() - Field.LinesVertical.starting;
            slopeDir = 1.0;

        } else {
            double redInitLineX = Field.fieldLength - Field.LinesVertical.starting;
