...
public class RobotSim {
    ...
    public static Trigger simLaunching() {
        return new Trigger(() -> Utils.isSimulation()
                && (Robot.getSuperStructure().getCurrentSuperState() == CurrentSuperState.LAUNCH_WITH_SQUEEZE
                        || Robot.getSuperStructure().getCurrentSuperState() == CurrentSuperState.LAUNCH_WITHOUT_SQUEEZE
                        || Robot.getSuperStructure().getCurrentSuperState()
                                == CurrentSuperState.LAUNCH_WITH_SQUEEZE_WITH_NO_DELAY));
    }

    @Getter
    private static double simRobotWidth = Units.inchesToMeters(33);

    @Getter
    private static double simRobotLength = Units.inchesToMeters(32.75);
    ...
    public void updateArticulatedMechanisms() {
        double intakeExtensionPose = Units.inchesToMeters(12)
                * robotSuperStructure.getIntakeExtension().getPositionPercentage()
                / 100;
        var intakePose3d =
                Pose3d.kZero.plus(new Transform3d(new Translation3d(intakeExtensionPose, 0, 0), Rotation3d.kZero));

        double hoodAngleDegrees = robotSuperStructure.getHood().getPositionDegrees();
        var hoodPose3d = Pose3d.kZero.rotateAround(
                SHOOTER_HOOD_PIVOT_POINT, new Rotation3d(0, -Math.toRadians(hoodAngleDegrees - 9), 0));
    ...
    public void drawSideRobot() {
        MechanismRoot2d robotRoot = leftView.getRoot(
                "Top Robot Root",
    ...
    private void configBallSimRobot() {
        ...
        ballSim.addIntakeZone(
                ...
                intakeYMin,
                intakeYMax,
                () -> Robot.getSuperStructure().getCurrentSuperState() == CurrentSuperState.INTAKE_FUEL);
    }
    ...
    private Command createSimBallLaunch(double laneOffset) {
        return Commands.runOnce(() -> {
            ...
            Transform2d robotToLauncher = new Transform2d(
                    new Translation2d(Units.inchesToMeters(-5.5), Units.inchesToMeters(laneOffset)),
                    Rotation2d.k180deg);
            Translation3d launcherPose = new Translation3d(
        ...
        return Commands.defer(
                () -> {
                    ...
                    for (int i = 0; i < numToLaunchPerLane; i++) {
                        group1.addCommands(createSimBallLaunch(lane1), Commands.waitSeconds(timeBetweenBallLaunches));
                    }
                    SequentialCommandGroup group2 =
                            new SequentialCommandGroup(Commands.waitSeconds(Math.random() * 0.3));
                    for (int i = 0; i < numToLaunchPerLane; i++) {
                        group2.addCommands(createSimBallLaunch(lane2), Commands.waitSeconds(timeBetweenBallLaunches));
                    }
                    SequentialCommandGroup group3 =
                            new SequentialCommandGroup(Commands.waitSeconds(Math.random() * 0.3));
                    for (int i = 0; i < numToLaunchPerLane; i++) {
                        group3.addCommands(createSimBallLaunch(lane3), Commands.waitSeconds(timeBetweenBallLaunches));
                    }
                    SequentialCommandGroup group4 =
                            new SequentialCommandGroup(Commands.waitSeconds(Math.random() * 0.3));
                    for (int i = 0; i < numToLaunchPerLane; i++) {
                        group4.addCommands(createSimBallLaunch(lane4), Commands.waitSeconds(timeBetweenBallLaunches));
                    }
                    return Commands.parallel(group1, group2, group3, group4).withName("RobotSim.ballSimLaunchFuel");
                },
